// From Hummingbird
// TODO: Remove if LocalFileSystem is removed
...
public struct FileIO: Sendable {
  ...
  public func writeFile<AS: AsyncSequence>(
    ...
    try await self.fileIO.withFileHandle(path: path, mode: .write, flags: .allowFileCreation()) {
      handle in
  ...
  func readFile(path: String, range: ClosedRange<Int>, context: some BaseRequestContext)
    -> ResponseBody
  {
