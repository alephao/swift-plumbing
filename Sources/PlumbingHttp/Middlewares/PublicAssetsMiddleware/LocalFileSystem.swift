// From Hummingbird
// TODO: Remove if LocalFileSystem.init becomse public public
...
public struct LocalFileSystem {
    ...
    do {
      ...
      let isFolder = (lstat.st_mode & S_IFMT) == S_IFDIR
        ...
        let modificationDate =
          Double(lstat.st_mtim.tv_sec) + (Double(lstat.st_mtim.tv_nsec) / 1_000_000_000.0)
      #else
        let modificationDate =
          Double(lstat.st_mtimespec.tv_sec) + (Double(lstat.st_mtimespec.tv_nsec) / 1_000_000_000.0)
      #endif
      return .init(
        isFolder: isFolder,
  ...
  ///   - context: Request context
  /// - Returns: Response body
  public func loadFile(path: String, range: ClosedRange<Int>, context: some BaseRequestContext)
